          ...
          <div className="flex items-center justify-between h-16">
            ...
            <Link to="/" className="flex items-center gap-2.5">
              <motion.div whileHover={{ scale: 1.08, rotate: -5 }}
                ...
                style={{ background: '#00d4ff', boxShadow: '0 0 18px rgba(0,212,255,0.4)' }}>
                G
              </motion.div>
            ...
            <div className="hidden md:flex items-center gap-0.5">
              {navLinks.map((link) => {
                ...
                return (
                  <Link key={link.path} to={link.path}
                    className="relative px-3.5 py-2 rounded-lg font-body text-sm font-medium transition-colors duration-200"
              ...
              {/* Theme toggle — no admin link */}
              <motion.button onClick={toggleTheme}
                whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.92 }}
                className="ml-3 w-10 h-10 rounded-xl flex items-center justify-center overflow-hidden"
            ...
            <div className="md:hidden flex items-center gap-2">
              ...
              <button onClick={() => setOpen(!open)} className="p-2 rounded-lg" style={{ color: colors.textMuted }}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            style={{ position: 'fixed', top: 64, left: 0, right: 0, zIndex: 40, background: colors.navBg, borderBottom: `1px solid ${colors.navBorder}`, backdropFilter: 'blur(20px)' }}
            className="md:hidden">
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link key={link.path} to={link.path}
                    ...
                    style={{ color: active ? '#00d4ff' : colors.textMuted, background: active ? 'rgba(0,212,255,0.08)' : 'transparent', border: active ? '1px solid rgba(0,212,255,0.2)' : '1px solid transparent' }}>
                    {link.name}
                  </Link>
                );
              })}
