    ...
    <div className="min-h-screen pt-24 pb-20" style={{ background: colors.bg2 }}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeader tag="Contact" title="Let's Work Together"
          subtitle="Have a project in mind? Drop me a message and I'll get back to you within 24 hours!" />
          ...
          <div className="lg:col-span-2 space-y-4">
            {contactInfo.map(({ icon: Icon, label, value, href }, i) => (
              <motion.div key={label}
                ...
                <div>
                  ...
                  {href
                    ? <a href={href} className="font-body text-sm font-medium hover:text-primary transition-colors" style={{ color: colors.text }}>{value}</a>
                    : <p className="font-body text-sm font-medium" style={{ color: colors.text }}>{value}</p>}
                </div>
              </motion.div>
            ))}
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }}
              ...
              <p className="font-body text-sm leading-relaxed" style={{ color: colors.textMuted }}>
                Open to freelance, full-time roles, and collaborations. Usually reply within 24 hours.
              </p>
              <div className="mt-3 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                <span className="text-green-600 font-body text-xs font-semibold">Available for work</span>
              </div>
            </motion.div>
          </div>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }}
            ...
            <div className="card gradient-border">
              ...
              <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="label">Phone <span style={{ color: colors.textDim }}>(optional)</span></label>
                    <input type="tel" name="phone" value={form.phone} onChange={handleChange}
                      placeholder="+91 73047 85968" className="input-field" />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <textarea name="message" value={form.message} onChange={handleChange}
                    placeholder="Tell me about your project or say hello..." rows={5}
